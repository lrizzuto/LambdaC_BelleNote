...
fixed and only the the normalisations of \textbf{reconstructed}  and \textbf{misreconstructed signal} were floated. \\
 ...
 As an example \cref{fig:10streams_chargedBtag_corrLambdaC_TotalSignalBtag_fit} shows the fit performed on the tagged $B$ mesons having in the ROE a companion decaying $B^- \rightarrow \Lambda_c^+$ or  
 $B^+ \rightarrow \Lambda_c^-$ (charged correlated decays). 
 Dividing the yields obtained by the fit by the total number of charged correlated decays events present in the 10 Monte Carlo streams it is possible to determine the FEI tag-side efficiency for signal events.
 \begin{figure}[H]
    \centering
...
The efficiency of reconstructing the ${\Lambda_c}$ baryon after correctly tagging the  $B$ meson, can be estimated from Monte Carlo simulated data.
One can again exploit the  10 streams of on-resonance Monte Carlo simulated data for to estimate the $\Lambda_c$ reconstruction efficiency.
...
\noindent where $N_{recSig}(B_{tag}, \Lambda_c))$ are the yields of reconstructed signal from a two dimensional fit of the corresponding inclusive decay
...
UNMIXED:
$\epsilon^0_{FEI,sig} = 0.247 \pm 0.002\%$ (calculated with  10 streams)\\
$\epsilon^0_{FEI} = 0.234 \pm 0.0002\%$ \\

using only unmixed:\\
$\frac{\epsilon^0_{FEI,sig}}{\epsilon^0_{FEI}}$ = 1.056 $\pm$ 0.009% 𝜖𝐹𝐸𝐼=  0.234 ± 0.001% , 𝜖𝐹𝐸𝐼,𝑠𝑖𝑔= 0.247 ± 0.002%

Including mixed events:
$\epsilon^0_{FEI,sig} = 0.253 \pm 0.002\%$ (calculated with  10 streams)\\
$\epsilon^0_{FEI} = 0.239 \pm 0.0002\%$ \\
$\frac{\epsilon^0_{FEI,sig}}{\epsilon^0_{FEI}}$ = 1.059 $\pm$ 0.006

$\epsilon_{\Lambda_c}$ = 39.59 $\pm $ 0.49 $\%$ %overall correction factor 0.794 $\pm$ 0.010
 (using unmixed events and calculated with  10 streams)

 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{$\bar{B^0} \rightarrow \Lambda_c^-$  decays}
UNMIXED:
$\epsilon^0_{FEI,sig} = 0.210 \pm 0.003\%$ (calculated with  6 streams unmixed)\\
$\epsilon^0_{FEI} = 0.207 \pm 0.0002\%$ \\
using only unmixed:\\
$\frac{\epsilon^0_{FEI,sig}}{\epsilon^0_{FEI}}$ = 1.017 $\pm$ 0.015\\

Including mixed events:
$\epsilon^0_{FEI,sig} = 0.207 \pm 0.003\%$ (calculated with 6 streams)\\
$\epsilon^0_{FEI} = 0.208 \pm 0.0002\%$ \\
$\frac{\epsilon^+_{FEI,sig}}{\epsilon^+_{FEI}}$ = 0.995 $\pm$ 0.014\\% 𝜖𝐹𝐸𝐼= 0.206 ± 0.001% , 𝜖𝐹𝐸𝐼,𝑠𝑖𝑔= 0.207 ± 0.003%

$\epsilon_{\Lambda_c}$ = 41.49  $\pm $ 1.13 $\%$\\ %overall correction factor 0.790 $\pm$ 0.011
(using unmixed events and calculated with  6 streams, unfortunately didn't run on all streams within 2023)