...
\subsection{Total Signal fits}\label{subsec:sigBtagFit}
...
As in the 2D fit, the parameter for the width, $\sigma_{CB}$, of the Crystal Ball is usually floated. In the case of charged correlated 
decays an exception was made (the reason is given in SubSec:\ref{subsec:chargedAntiBtagFit}): the parameter of the signal PDF which is floated 
...
To avoid introducing significant systematic uncertainties in the fit deriving from the $M_{bc}$ endpoint region, 
where one has a smearing effect due to variations of the beam energy at the $MeV$ level, the range for the fit is restricted to
  values betweeen 5.250 and 5.287 GeV/c$^2$ (as one could already notice in the signal sample fits in SubSec:\ref{subsec:sigBtagFit}).
...
\begin{tabular}{ |p{2.5cm}||p{4.2cm}| p{4.2cm}|  }
 ...
 \hline
 NrecSig  & 4.77874 $\cdot$10 $^6 \pm$ 6748 &  4.7571 $\cdot$10 $^6 \pm$ 3214 \\
 NmisSig &  5.39870 $\cdot$10 $^6 \pm$ 5617 & 5.4035 $\cdot$10 $^6 \pm$ 3376 \\
 \hline
\end{tabular}
...
The study shows a possible small bias the reconstructed signal yields  (see \cref{fig:NrecSignalchargedCorrBtag_mcstudy}).


\begin{figure}[H]
    \centering
    {\centering\includegraphics[width=15cm]{05-BtagFit/figs/NrecSig_mcstudy.png}}
...
\subsection{Fit results for $B^- \rightarrow \bar{\Lambda}_c^-$ decays}\label{subsec:chargedAntiBtagFit}  

As already mentioned, in the case of this decay channel, the startegy for the $B_{tag}$ fit had to be slightly changed. The reason is
easily explained. The fit performed with same floated parameter as in the previous decay channel, shown in Fig. \cref{fig:chargedAntiLambdaC_BtagFit-sigmaCB},
converges and results are reasonable (see the table down here below); unfortunately in 60$\%$ of the fits performed in the toy MC study failed and the converged fits 
show as well some not negligible bias in the signal yields (see \cref{fig:NrecSigchargedAntiBtag_mcstudy_sigmaCB_free}).\\
Here below the reconstructed/misreconstructed signal yields from the fit having $\sigma_{CB}$ floated are reported and compared with the expected values.
One can notice a very large uncertainty on the reconstructed signal, which is already an indication of troubles arising from the not good choice of floating parameters.
...
\begin{tabular}{ |p{2.5cm}||p{4.2cm}| p{4.2cm}|  }
    \hline
             &    Fit results                 &  expected\\
    \hline
    NrecSig  & 2.58006$\cdot$10$^6 \pm$ 11796 &  2.58692 $\cdot$10$^6 \pm$ 1858 \\
    NmisSig &  7.22503$\cdot$10$^5 \pm$ 7271 & 7.13865 $\cdot$10$^5 \pm$ 1274 \\
    \hline
   \end{tabular}
...
\begin{figure}[H]
    ...
    \caption{Total fit of tagged $B$ mesons on Monte Carlo simulated data.}
    \label{fig:chargedAntiLambdaC_BtagFit-sigmaCB}
    \end{figure}

\begin{figure}[H]
\centering
{\includegraphics[width=15cm]{05-BtagFit/figs/NrecSigchargedAntiBtag_mcstudy_sigmaCB_free.png}}
    \caption{Plots showing distributions of the fitted signal yields, errors and the pull distribution of toy Monte Carlo pseudo-fits 
    where the width of the Crystal Ball is floated.}
    \label{fig:NrecSigchargedAntiBtag_mcstudy_sigmaCB_free}
    \end{figure}

    \begin{figure}[H]
        \centering
        {\includegraphics[width=0.50\textwidth]{05-BtagFit/figs/NEW_chargedBtag_anticorrLambdaC_Total_fit_sigmaGauss_misRecoSlope_free_370bins.png}}
        \caption{Total fit of tagged $B$ mesons on Monte Carlo simulated data with sigmaGauss free.}
        \label{fig:chargedAntiLambdaC_BtagFit-sigmaGauss}
        \end{figure}
   % \vspace{1.5cm} 
   ...
   %     \hline
   %     NrecSig  & 2.58006$\cdot$10$^6 \pm$ 11796\\
   %     NmisSig &  7.22503$\cdot$10$^5 \pm$ 7271 \\
   %     \hline
   %    \end{tabular}
...
%with sigmaCB:
   ...
   \begin{tabular}{ |p{2.5cm}||p{4.2cm}| p{4.2cm}|  }
    \hline
             &    Fit results                 &  expected\\
    \hline
    NrecSig  & 2.60316$\cdot$10$^6 \pm$ 3681 &  2.58692 $\cdot$10$^6 \pm$ 1858 \\
    NmisSig &  7.10860$\cdot$10$^5 \pm$ 2421 & 7.13865 $\cdot$10$^5 \pm$ 1274 \\
    \hline
   \end{tabular}   

       \vspace{0.5 cm}
       \noindent The Total Signal (the sum  NrecSig+NmisSig) calculated by the fit is 3302559 $\pm$ 4914, which is in excellent agreement with 
       the true Monte Carlo value: 3299629.

\begin{figure}[H]
\centering
{\includegraphics[width=15cm]{05-BtagFit/figs/NrecSigchargedAntiBtag_mcstudy_sigmaGauss_free.png}}
\caption{Plots showing distributions of the fitted signal yields, errors and the pull distribution of toy Monte Carlo pseudo-fits 
where the width of the Gaussian is floated.}
\label{fig:NrecSigchargedAntiBtag_mcstudy_sigmaCB_free}
\end{figure}
...
\begin{figure}[H]
    \centering
    {\includegraphics[width=0.50\textwidth]{05-BtagFit/figs/stream1_CorrNeutralBtagNEWsignalPDF_Total_fit_sigmaCB_misReco_slope_370bins.png}}
    \caption{Total fit of tagged $B^0$ mesons on Monte Carlo simulated data.}
...
Reconstructed and misreconstructed signal yields obtained from the fit:\\
    ...
    \begin{tabular}{ |p{2.5cm}||p{4.2cm}| p{4.2cm}|  }
        \hline
                 &    Fit results       &  expected\\
        \hline
     NrecSig  & 1.7336 $\cdot$10 $^6 \pm$ 23370  & 1.7519 $\cdot$10 $^6  \pm$ 3941 \\
     NmisSig &  5.3446 $\cdot$10 $^5 \pm$ 1435  & 5.3796 $\cdot$10 $^5  \pm$  3786\\
     \hline
    \end{tabular}      
    
    \vspace{0.5 cm}
    \noindent The Total Signal (the sum  NrecSig+NmisSig) is 2268012 $\pm$ 1747 (to be compared with 2286964 from the Monte Carlo).
...
\subsection{Fit results for $B^0 \rightarrow {\Lambda}_c^+$ decays} 



\begin{figure}[H]
    \centering
    {\includegraphics[width=0.50\textwidth]{05-BtagFit/figs/NEW_neutralBtag_Total_fit_sigmaCB_misReco_slope_free_370bins.png}}
    \caption{Total fit of tagged $B$ mesons on Monte Carlo simulated data.}
...
Reconstructed and misreconstructed signal yields obtained from the fit:\\
    ...
    \begin{tabular}{ |p{2.5cm}||p{4.2cm}| p{4.2cm}|  }
        \hline
                 &    Fit results       &  expected\\
     \hline
     NrecSig  & 1.54229$\cdot$10 $^6 \pm$ 3269  &  1.54945$\cdot$10 $^5 \pm$ 1396\\
     NmisSig &  3.71731$\cdot$10 $^5 \pm$ 4872  &  3.70506$\cdot$10 $^5 \pm$ 886 \\
     \hline
    \end{tabular}      
 
   
 \vspace{0.5 cm}
 \noindent The Total Signal (the sum  NrecSig+NmisSig) is 1914022 $\pm$ 2330 (to be compared with 1920156 from the Monte Carlo).
One can find some undersestimation in the Signal yiels (about 2$\sigma$) and a more prominent underestimation in the estimated Total Signal, about $\sim 3.7\sigma$. 
 
        
The toy MC study result for the signal yields is shown in \cref{fig:NrecSig_neutralAntiBtag_mcstudy}.
 No hints of bias are visible in this case.
