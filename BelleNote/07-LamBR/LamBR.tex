...
\section{charged decays branching ratios}

Once all the factors of \cref{eq:chargedB_BRformula} are obtained from the MOnte Carlo, the corresponding branching ratios can be calculated in each stream (6 streams in total, since there are only 6 available for the off-resonance). 
\vspace{0.75 cm}
\begin{table}[H]
    \centering
    ...
    \begin{table}[H]
        ...
        \toprule
         ...
         \midrule
         \hline
        stream 0	&	(1.23 $\pm$ 0.11)$\%$  &	(1.19 $\pm$ 0.05)$\%$	 &	(1.217 $\pm$ 0.002)$\%$\\
        stream 1	&	(1.23 $\pm$ 0.11)$\%$	&	(1.21 $\pm$ 0.05)$\%$	 &	(1.218 $\pm$ 0.002)$\%$	\\
        stream 2	&	(1.34 $\pm$ 0.12)$\%$	&	(1.23 $\pm$ 0.05)$\%$	 &	(1.218 $\pm$ 0.002)$\%$\\
        stream 3	&	(1.15 $\pm$ 0.10)$\%$	&	(0.20 $\pm$ 0.05)$\%$	 &	(1.215 $\pm$ 0.002)$\%$\\
        stream 4	&	(1.43 $\pm$ 0.12)$\%$   &	(1.21 $\pm$ 0.05)$\%$	 &	(1.218 $\pm$ 0.002)$\%$\\
        stream 5	&	(1.03 $\pm$ 0.11)$\%$	&	(1.15 $\pm$ 0.05)$\%$	 &	(1.217 $\pm$ 0.002)$\%$\\
        \midrule
        \hline
        average		&	(1.24 $\pm$	0.05)$\%$	&	(1.20 $\pm$	0.02)$\%$	& (1.217 $\pm$ 0.001)$\%$\\
        \bottomrule
        \hline
        \end{tabular}%
        }
    \label{tab:SixStreams_chargedAntiCorrLamBR}
    \caption{Monte Carlo measured branching fraction values of charged anticorrelated decays}
    %\caption{Measured branching fraction values obtained using the results listed in \cref{tab:SixStreams_chargedCorrLam2Dfits} for the six different streams (only statistical uncertainties are displayed) and its average.}
    \end{table}
...
Due to the mixing phenomenon of neutral $B$ mesons the measurement requires an intermediate step in order to correct the production 
 rates of $\Lambda_c$ baryons obtained in the rescoil of the reconstructed neutral $B$ mesons for the $B^0\bar{B^0}$ mixing. \\
...
\begin{equation}
    \begin{cases}
        ...
        \bar{\mathcal{B}}_c = \dfrac{(\bar{B}^0_{tag}, \Lambda_c^-) - \chi_d (\mathcal{B}(\bar{B}^0_{tag}, \Lambda_c^-) +  \mathcal{B}(B^0_{tag}, \Lambda_c^-))}{1 - 2\chi_d}
    \end{cases}  
\end{equation}

First a check of the decays that didn't experience mixing is performed. For this check, samples of signal events that didn't experience mixing
were used to obtain  signal yields (see Figures below).

\begin{figure}[H]
    \centering
    {\includegraphics[width=0.7\textwidth]{07-LamBR/figs/stream4_TotalSignal_neutral_corrLambdaC_2Dfit.png}}
    \caption{Unmixed neutral correlated total signal events in $M_{bc}$  and $M(p K \pi)$ }
    \label{fig:stream4_TotalSignal_neutral_corrLambdaC_2Dfit}
    \end{figure}

    \begin{figure}
        \centering
        {\includegraphics[width=0.7\textwidth]{07-LamBR/figs/stream3_TotalSignal_neutral_anticorrLambdaC_2Dfit.png}}
        \caption{Unmixed neutral anticorrelated total signal events in $M_{bc}$  and $M(p K \pi)$ }
        \label{fig:stream3_TotalSignal_neutral_anticorrLambdaC_2Dfit}
        \end{figure}

\begin{figure}[H]
    \centering
    {\includegraphics[width=0.40\textwidth]{07-LamBR/figs/stream1_neutralBtag_unmixedTotal_Signal_fit_normalisations.png}}
    \caption{Unmixed neutral $B$ mesons in the correlated decay sample. }
    \label{fig:anticorr_chargedBtag_Total_Signal_fit}
    \end{figure}

    \begin{figure}[H]
        \centering
        {\includegraphics[width=0.40\textwidth]{07-LamBR/figs/stream1_unmixedTotalSignal_BtagFit_anticorrLambdaC_New_signalPDF.png}}
        \caption{Unmixed neutral $B$ mesons in the anticorrelated decay sample. }
        \label{fig:anticorr_chargedBtag_Total_Signal_fit}
        \end{figure}


\subsection{unmixed neutral decays branching ratios}
The following two tables report the values of branching fractions for the neutral decays' events that didn't experience mixing 
(i.e.: $\mathcal{B}_c $ and $\bar{\mathcal{B}}_c$), 
obtained with the fits shown in 
\begin{table}[H]
    \centering
...
The good agreement within statistical errors of the obtained branching fractions excluding mixing events guarantees that 
the procedure to obtain the branching fractions is correct and any possible discrepancy observed when mixing is included does not
depend on it.
\newpage
 . . . Adding mixed events:
 \newline
 \begin{itemize}
    \item Neutral correlated decays
\end{itemize}
\begin{table}[H]
    \centering
    \resizebox{0.95\textwidth}{!}{%
    ...
    \toprule
     ...
     \midrule
     \hline
    stream 0	&	(3.74 $\pm$ 0.26)$\%$  &	(3.41 $\pm$ 0.14)$\%$	 &	(3.439 $\pm$ 0.003)$\%$\\
    stream 1	&	(3.69 $\pm$ 0.26)$\%$	&	(3.65 $\pm$ 0.14)$\%$	 &	(3.431 $\pm$ 0.003)$\%$	\\
    stream 2	&	(3.53 $\pm$ 0.25)$\%$	&	(3.42 $\pm$ 0.14)$\%$	 &	(3.433 $\pm$ 0.003)$\%$\\
    stream 3	&	(3.58 $\pm$  0.25)$\%$	&	(3.43 $\pm$ 0.14)$\%$	 &	(3.433 $\pm$ 0.003)$\%$\\
    stream 4	&	(3.58 $\pm$  0.25)$\%$ &   (3.34 $\pm$ 0.14)$\%$	 &	(3.434 $\pm$ 0.003)$\%$\\
    stream 5	&	(3.57 $\pm$  0.26)$\%$	&	(3.37 $\pm$ 0.14)$\%$	 &	(3.436 $\pm$ 0.003)$\%$\\
    \midrule
    \hline
    average			&	(3.58 $\pm$	0.10)$\%$	&	(3.44 $\pm$	0.06)$\%$	& (3.434 $\pm$ 0.001)$\%$\\
    \bottomrule
    \hline
    \end{tabular}%
    }
    \caption{Measured branching fraction values obtained using the results listed in  for the six different streams (only statistical uncertainties are displayed) and its average.}
    \end{table}
    xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\newline
\begin{itemize}
    \item Neutral anticorrelated decays
\end{itemize}
...
\begin{table}[H]
    ...
    \toprule
     ...
     \midrule
     \hline
    stream 0	&	(1.13 $\pm$ 0.13)$\%$  &	(1.22 $\pm$ 0.07)$\%$	 &	(1.300 $\pm$ 0.002)$\%$\\
    stream 1	&	(1.13 $\pm$ 0.13)$\%$	&	(1.27 $\pm$ 0.07)$\%$	 &	(1.299 $\pm$ 0.002)$\%$	\\
    stream 2	&	(1.20 $\pm$ 0.13)$\%$	&	(1.28 $\pm$ 0.08)$\%$	 &	(1.300 $\pm$ 0.002)$\%$\\
    stream 3	&	(1.67 $\pm$ 0.16)$\%$	&	(1.29 $\pm$ 0.07)$\%$	 &	(1.299 $\pm$ 0.002)$\%$\\
    stream 4	&	(1.33 $\pm$ 0.15)$\%$ &   (1.20 $\pm$ 0.07)$\%$	 &	(1.295 $\pm$ 0.002)$\%$\\
    stream 5	&	(1.24 $\pm$ 0.13)$\%$	&	(1.30 $\pm$ 0.08)$\%$	 &	(1.298 $\pm$ 0.002)$\%$\\
    \midrule
    \hline
    average			&	(1.28 $\pm$	0.06)$\%$	&	(1.26 $\pm$	0.03)$\%$	& (1.299 $\pm$ 0.001)$\%$\\
    \bottomrule
    \hline
    \end{tabular}%
    }
...
As discussed in \cref{sec:MC2D_Fit} the shapes of the PDFs describing the generic background (peaking or flat in $M_{bc}$ ) are kept fixed
 ...
 The tracking efficiency correction for tracks with $p > $ 0.2 GeV is studied in BELLE NOTE 1165.% \cite{TrackingEff}  . 
The track finding efficiency is measured by comparing the number of partially and fully reconstructed
 ...
 than the statistical uncertainty, this ratio is not applied as a correction, instead, as done in BELLE NOTE 1584, a systematic uncertainty of 0.35$\%$ per track is applied. The total systematic uncertainty is the sum over the 
...
already described in \ref{sec:continuumSysBtoLambdaC} one obtains an absolute value of $0.04\%$ on the branching fraction.\\
...
The $\Lambda_c$ reconstruction efficiency is determined to be $\epsilon_{\Lambda_c}$ = 37.78 $\pm 0.45 \%$. 
The systematic uncertainty originated by its uncertainty is 0.04$\%$ on the Br value.\\
...
overall correction factor: $0.813 \pm 0.013$
\noindent The final PID efficiency systematic error is determined to be  0.05$\%$ on the branching fraction value. \\
The systematic uncertainty of 0.35$\%$ per track for the tracking efficiency instead results in 0.01$\%$ uncertainty on the branching fraction value. 
...
\begin{table}[h]
    ...
    one obtains an absolute value of $0.01\%$ on the branching fraction.\\
    Instead the imperfect modeling of the continuum in the $B_{tag}  $ fit is responsible for a 0.02 $\%$ on the branching fraction in absolute value.\\
    Continuum modeling uncertainties account for 0.02$\%$ total uncertainties on the branching fraction in absolute value.\\  
    
    Regarding the crossfeed/misreconstructed signal ratio in the $B_{tag}  $ fit, the produced offsets are displayed here below.
    \vspace{0.25 cm}
    \begin{table}[H]
    \begin{tabular}{ |p{2.5cm}||p{2cm}| p{2cm}|  p{2cm}|}
    \hline
    ...
    The offsets that a potential $\pm$20$\%$ different crossfeed/misreconstructed signal ratio produce results in 0.01$\%$ systematic
    uncertainity on the branching fraction, as in the charged correlated decays.\\
    
    The systematic uncertainties deriving from the crossfeed background modeling in the $B_{tag}$ fit is    about 0.02$\%$.
    In the table below the signal yields' offsets are listed changing
    the parameters within their uncertainties, and the mean offsets value used to calculate the
    expected uncertainty on the BR value.
    \vspace{0.25 cm}
    \begin{table}[H]
    \begin{tabular}{ |p{2.5cm}||p{2cm}| p{2cm}|  p{2cm}|}
    \hline
    ...
    \begin{table}[h]
        ...
        source              & relative ($\%$) & absolute ($\%$)  \\
        \hline
        Continuum modeling   &  1.61   &  0.02 \\
        $B_{tag}$ fit Crossfeed fraction   &  0.12 &  0.01 \\
        $B_{tag}$ fit Crossfeed PDF shape & 1.93 &  0.02  \\
        2DFit crossfeed normalization & 3.23 & 0.04 \\
        $M_{bc}$ peaking PDF shape & 0.80 &  0.01  \\
        $M_{bc}$ flat PDF shape & - &  - \\
        
        $\epsilon^{+}_{FEI,  sig}/\epsilon^{+}_{FEI}$ & 0.81 & 0.01 \\
        $\epsilon_{\Lambda_c}$ & 1.61 & 0.02 \\
        PID            & 1.26    & 0.01\\
        Tracking efficiency & 0.92  & 0.01 \\
        \hline
        Total                         &   4.80    &  0.06  \\
        \hline
        \end{tabular}%
        }%
        \caption{Systematic uncertainties in the determination of the  $B^+ \rightarrow \bar{\Lambda}_c^+ X$ branching fraction.}
        \label{tab:systematics:ChargedAnti}
        \end{table}
...
\begin{table}[h]
    ...
    source              & relative ($\%$) & absolute ($\%$)  \\
    \hline
    Continuum modeling   &  1.65   &  0.06 \\
    $B_{tag}$ fit Crossfeed fraction   &  0.27 &  0.01 \\
    $B_{tag}$ fit Crossfeed PDF shape & 0.55 &  0.02  \\
    2DFit crossfeed normalization & 0.27 & 0.01 \\
    $M_{bc}$ peaking PDF shape & 0.27 &  0.01  \\
    $M_{bc}$ flat PDF shape & / &  / \\ 
    $\epsilon^{+}_{FEI,  sig}/\epsilon^{+}_{FEI}$ & 0.82 & 0.03 \\
    $\epsilon_{\Lambda_c}$ & 1.37 & 0.05 \\
    PID            & 1.37 & 0.05 \\
   Tracking efficiency & 1.10  & 0.04 \\
   Mixing parameter & 0.27  & 0.01 \\
    \hline
    Total                         &   3.02     &  0.11  \\
    \hline
    \end{tabular}%
    }%
    \caption{Systematic uncertainties in the determination of the  $\bar{B^0} \rightarrow \Lambda_c^+ X$ branching fraction in \si{\percent}.}
    \label{tab:systematics:NeutralCorr}
    \end{table}
...
\newpage
    ...
    \begin{table}[h]
        ...
        source              & relative ($\%$) & absolute ($\%$)  \\
        \hline
        Continuum modeling   &  1.64   &  0.02 \\
        $B_{tag}$ fit Crossfeed fraction   &  0.82 &  0.01 \\
        $B_{tag}$ fit Crossfeed PDF shape & 1.64 &  0.02  \\
        2DFit crossfeed normalization & 0.74 & 0.01 \\
        $M_{bc}$ peaking PDF shape & 3.28 &  0.04  \\
        $M_{bc}$ flat PDF shape & 0.82 &  0.01 \\ 
        $\epsilon^{+}_{FEI,  sig}/\epsilon^{+}_{FEI}$ & 3.28 &  0.04 \\
        $\epsilon_{\Lambda_c}$ & 1.64 &  0.02 \\
        PID            & 11.07 & 0.14 \\
       Tracking efficiency & 2.46  & 0.03 \\
       Mixing parameter & 0.82  & 0.01 \\
        \hline
        Total                         &   8.59     &  0.16  \\
        \hline
        \end{tabular}%
        ...
        \caption{Systematic uncertainties in the determination of the  $\bar{B^0} \rightarrow \Lambda_c^+ X$ branching fraction in \si{\percent}.}
        \label{tab:systematics:NeutralAnti}
        \end{table}
